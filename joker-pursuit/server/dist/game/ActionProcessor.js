...
const hasValidGameOverWinner = (nextState) => {
    const winner = nextState.winner;
    if (!isNonEmptyObject(winner)) {
        return false;
    }
    return (typeof winner.playerId === 'string' &&
...
class ActionProcessor {
    process(input) {
        ...
        if (!Number.isInteger(nextPlayerIndex) ||
            nextPlayerIndex < 0 ||
        ...
        const isGameOverCommit = nextState.phase === 'gameOver';

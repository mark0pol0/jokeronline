...
const ActionProcessor_1 = require("./game/ActionProcessor");
...
const roomStorePromise = (0, RoomStore_1.createRoomStore)();
const actionProcessor = new ActionProcessor_1.ActionProcessor();
...
function generatePlayerId() {
    return `player-${Date.now()}-${(0, crypto_1.randomBytes)(2).toString('hex')}`;
}
function generateSessionToken() {
    return (0, crypto_1.randomBytes)(24).toString('hex');
}
function serializeV2Players(players) {
...
function findCardInState(gameState, cardId) {
...
function summarizeMoveMetadata(metadata) {
...
function summarizeMove(move) {
    ...
    const destinations = Array.isArray(moveObject.destinations)
        ? moveObject.destinations.filter((destination) => typeof destination === 'string')
        : [];
    return {
...
function summarizeActionTransition(action, previousGameState, nextGameState) {
...
function buildSnapshot(room, selfPlayerId) {
    return {
        roomCode: room.code,
        roomId: room.id,
        stateVersion: room.stateVersion,
...
function isSessionForRoom(session, roomCode) {
    return session.roomCode.toUpperCase() === roomCode.toUpperCase();
}
function setSocketSession(socketId, session) {
    socketSessions.set(socketId, session);
}
function clearSocketSession(socketId) {
    socketSessions.delete(socketId);
}
async function ensureSessionBoundToSocket(store, room, session, socket) {
...
async function emitPresenceUpdate(room) {
    io.to(room.id).emit('presence-updated-v2', {
        roomCode: room.code,
        playersPresence: getPresenceMap(room)
    });
}
async function emitHostUpdate(room) {
...
async function sendRejected(socketId, reason, expectedVersion, room, selfPlayerId) {
...
io.on('connection', (socket) => {
    ...
    if (ENABLE_PROTOCOL_V2) {
        socket.on('create-room-v2', async (data, callback) => {
            try {
        ...
        socket.on('join-room-v2', async (data, callback) => {
            try {
        ...
        socket.on('rejoin-room-v2', async (data, callback) => {
            try {
                const roomCode = data?.roomCode?.trim()?.toUpperCase();
        ...
        socket.on('start-game-v2', async (data, callback) => {
            try {
                const roomCode = data?.roomCode?.trim()?.toUpperCase();
                const sessionToken = data?.sessionToken?.trim();
                if (!roomCode || !sessionToken) {
        ...
        socket.on('update-player-color-v2', async (data, callback) => {
            try {
                const roomCode = data?.roomCode?.trim()?.toUpperCase();
                const sessionToken = data?.sessionToken?.trim();
                ...
                if (room.gameState?.players?.length) {
                    room.gameState.players = room.gameState.players.map((statePlayer) => statePlayer.id === player.id
                        ? { ...statePlayer, color }
                        : statePlayer);
                }
                room.updatedAt = getNow();
        ...
        socket.on('submit-action-v2', async (data, callback) => {
            try {
                const roomCode = data?.roomCode?.trim()?.toUpperCase();
                const sessionToken = data?.sessionToken?.trim();
        ...
        socket.on('request-sync-v2', async (data, callback) => {
            try {
                const roomCode = data?.roomCode?.trim()?.toUpperCase();
                const sessionToken = data?.sessionToken?.trim();
                if (!roomCode || !sessionToken) {
        ...
        socket.on('leave-room-v2', async (data, callback) => {
            try {
                const roomCode = data?.roomCode?.trim()?.toUpperCase();
                const sessionToken = data?.sessionToken?.trim();
                if (!roomCode || !sessionToken) {
